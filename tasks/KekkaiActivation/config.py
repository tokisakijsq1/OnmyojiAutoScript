...
class ActivationConfig(BaseModel):
    ...
    card_sort_no_fallback: bool = Field(default=False, description='严格排序模式(不回退): 开启后若遍历完整卡列表也没找到排序中的卡,'
                                                                  '本次不挂任何卡也不回退默认方式,推送通知提醒人工挂卡,'
                                                                  '并按失败间隔(failure_interval,默认10小时)后自动重试')
