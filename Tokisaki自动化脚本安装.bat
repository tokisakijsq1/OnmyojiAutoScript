...
if not exist "%ROOT%\toolkit\python.exe" (
    echo [1/5] 下载官方 easy-install 包（toolkit: Python 3.10 + Git）...
    if not exist "%DL_DIR%\easy.rar" (
...
if not exist "%ROOT%\config\deploy.yaml" (
    ...
    if exist "%DL_DIR%\OnmyojiAutoScript-easy-install\config\deploy.yaml" (
        ...
        echo [错误] 找不到模板 deploy.yaml，无法生成。
        goto :fail
    )
)
...
set "OUT=%~2"
set "MINSZ=%~3"
if defined CURL_PROXY_ARGS (
    curl -sL !CURL_PROXY_ARGS! --connect-timeout 15 --max-time 600 -o "%OUT%" "%URL%"
) else (
    curl -sL --connect-timeout 15 --max-time 600 -o "%OUT%" "%URL%"
)
...
set "MIN=%~2"
set "FSZ=0"
if exist "%F%" for %%A in ("%F%") do set "FSZ=%%~zA"
set /a MINB=%MIN%*1000000
if !FSZ! LSS !MINB! (
    if exist "%F%" del "%F%"
    exit /b 1
)
...
mkdir "%ROOT%" 2>nul
robocopy "%SRC%" "%ROOT%" /E /NFL /NDL /NJH /NJS >nul
if !ERRORLEVEL! GEQ 8 exit /b 1
rem 把快照还原成 git 仓库（保留完整历史，方便后续更新/rebase）
"%GIT%" -C "%ROOT%" init -q
"%GIT%" -C "%ROOT%" remote add fork "%FORK%"
"%GIT%" -C "%ROOT%" remote add origin "%OFFICIAL%"
"%GIT%" -C "%ROOT%" fetch --depth=50 fork %BRANCH%
if errorlevel 1 exit /b 1
"%GIT%" -C "%ROOT%" checkout -q -B %BRANCH% FETCH_HEAD
"%GIT%" -C "%ROOT%" branch --set-upstream-to=fork/%BRANCH% %BRANCH% 2>nul
echo        镜像下载并还原 git 仓库成功。
exit /b 0
